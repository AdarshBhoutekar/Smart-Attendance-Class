...
def subject_card(name, code, section, stats=None, footer_callback=None):
    html = (
        ...
        f'Code : <span style="background:#E0E3FF; color:#5865F2; padding:2px 8px; border-radius: 5px;">{code} | Section : {section}</span>'
        '</p>'
    )
    if stats:
        ...
        for icon, label, value in stats:
            html += (
                ...
                f'{icon} <b style="color:#111827;">{value}</b> <span style="color:#111827;">{label}</span></div>'
            )
